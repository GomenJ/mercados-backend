...
@app.route("/api/v1/mercado/capacidad-transferencia", methods=["POST"])
def submit_capacidad_transferencia_batch():
    ...
    try:
        ...
        if not isinstance(records_list, list):
         ...
         app.logger.warning(f"Failed to parse JSON for {model_name}: {e}")
         return jsonify({"status": "error", "message": "Failed to parse JSON list"}), 400

    summary["total_records_received"] = len(records_list)
    if not records_list:
        app.logger.info(f"Received empty batch list for '{model_name}'.")
        summary["inserted"] = 0
        return jsonify({"status": "success", "summary": summary, "errors": []}), 200
    ...
    for index, record_dict in enumerate(records_list):
        if not isinstance(record_dict, dict):
            app.logger.warning(f"Item at index {index} for '{model_name}' not a dictionary. Skipping.")
            summary["failed_validation"] += 1
    ...
    if objects_to_insert: # Only attempt commit if there are valid objects
        try:
            ...
            db.session.rollback()
            app.logger.exception( # Log the full traceback
